...
let editor = renderer["a" /* default */].create('<div class="note-editor note-frame card"/>');
let toolbar = renderer["a" /* default */].create('<div class="note-toolbar card-header" role="toolbar"/>');
...
let editable = renderer["a" /* default */].create('<div class="note-editable card-block" contentEditable="true" role="textbox" aria-multiline="true"/>');
...
let buttonGroup = renderer["a" /* default */].create('<div class="note-btn-group btn-group">');
let dropdown = renderer["a" /* default */].create('<div class="note-dropdown-menu dropdown-menu" role="list">', function ($node, options) {
  let markup = Array.isArray(options.items) ? options.items.map(function (item) {
    ...
    let option = _typeof(item) === 'object' ? item.option : undefined;
    let dataValue = 'data-value="' + value + '"';
    let dataOption = option !== undefined ? ' data-option="' + option + '"' : '';
...
let dropdownButtonContents = function dropdownButtonContents(contents) {
  return contents;
};

let dropdownCheck = renderer["a" /* default */].create('<div class="note-dropdown-menu dropdown-menu note-check" role="list">', function ($node, options) {
  let markup = Array.isArray(options.items) ? options.items.map(function (item) {
    let value = typeof item === 'string' ? item : item.value || '';
    let content = options.template ? options.template(item) : item;
...
let dialog = renderer["a" /* default */].create('<div class="modal note-modal" aria-hidden="false" tabindex="-1" role="dialog"/>', function ($node, options) {
  if (options.fade) {
    $node.addClass('fade');
  }
...
let popover = renderer["a" /* default */].create(['<div class="note-popover popover in">', '<div class="arrow"></div>', '<div class="popover-content note-children-container"></div>', '</div>'].join(''), function ($node, options) {
...
let ui_checkbox = renderer["a" /* default */].create('<div class="form-check"></div>', function ($node, options) {
...
let ui_ui = function ui(editorOptions) {
  return {
    editor: editor,
    toolbar: toolbar,
