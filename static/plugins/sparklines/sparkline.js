(function (root, factory) {
  ...
  Sparkline.prototype.draw = function (points) {
    ...
    let pxHeight = this.options.height || this.element.offsetHeight;
    this.canvas.height = pxHeight * this.ratio;
    this.canvas.style.height = pxHeight + 'px';

    let lineWidth = this.options.lineWidth * this.ratio;
    let offsetX = Math.max(this.options.dotRadius * this.ratio, lineWidth/2);
    let offsetY = Math.max(this.options.dotRadius * this.ratio, lineWidth/2);
    let width = this.canvas.width - offsetX * 2;
    let height = this.canvas.height - offsetY * 2;

    let minValue = Math.min.apply(Math, points);
    let maxValue = Math.max.apply(Math, points);
    let bottomValue = this.options.minValue != undefined ? this.options.minValue : Math.min(minValue, this.options.maxMinValue != undefined ? this.options.maxMinValue : minValue);
    let topValue = this.options.maxValue != undefined ? this.options.maxValue : Math.max(maxValue, this.options.minMaxValue != undefined ? this.options.minMaxValue : maxValue);
    let minX = offsetX;
    let maxX = offsetX;

    let x = offsetX;
    let y = getY.bind(points, bottomValue, topValue, offsetY, height);
    let delta = width / (points.length - 1);

    let dot = drawDot.bind(this, this.options.dotRadius * this.ratio, offsetX, width + offsetX);
    let line = drawLine.bind(this, offsetX, width + offsetX);
